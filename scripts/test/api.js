 ...
 * TEST_CHAIN=optimism-sepolia npm run test:api-v2
 * TEST_CHAIN=base-sepolia npm run test:api-v2
 * TEST_CHAIN=mantle-testnet npm run test:api-v2
 *
 * Available Chains:
 * ---------------
 * - optimism-sepolia
...
async function retryWithDelay(fn, retries = 3, delay = 5000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log(
        chalk.yellow(
          `\nAPI returned ${error.response?.status}. Retrying in ${
            delay / 1000
          } seconds... (Attempt ${i + 2}/${retries})`
        )
      );
...
async function findBlockWithTransactions(
  provider,
  startBlock,
  maxAttempts = 10
) {
  ...
  throw new Error(
    `No blocks with sufficient transactions found in last ${maxAttempts} blocks`
  );
}
...
async function main() {
  ...
  const provider = new ethers.JsonRpcProvider(
    process.env[SOURCE_CHAIN.rpcEnvKey]
  );
  ...
  const txHash =
    typeof block.transactions[1] === "string"
  ...
  if (!receipt.logs.length) {
    ...
    console.log(
      chalk.yellow(
        "Try running the script again to test with a different transaction."
      )
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const proofRequest = await retryWithDelay(
      async () => {
        return axios.post(
          POLYMER_API_URL,
          {
      ...
      3,
      2000
    ); // Reduced to 3 retries, 2 second delay
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (result.status === "complete" && result.proof) {
          ...
          const proofHex =
            "0x" + Buffer.from(result.proof, "base64").toString("hex");
          ...
          console.log(
            chalk.gray(
              "Note: Convert base64 to hex format when passing proof to on-chain functions"
            )
          );
