...
async function findBlockWithReceipts(provider, startBlock) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    currentBlock--;
    console.log(
      chalk.gray(">  No suitable receipts, trying previous block...")
    );
  }
...
async function main() {
  ...
  if (!chainConfig) {
    throw new Error(
      `Unsupported chain ID: ${chainId}. Supported chains: ${Object.values(
        CHAINS
      )
  ...
  // Find suitable block
  const { blockNumber, txIndex, logIndex } = await findBlockWithReceipts(
    provider,
    targetBlock
  );
    ...
    if (jobId) {
      ...
      while (Date.now() - startTime < timeoutMs) {
        ...
        if (statusResponse?.result?.status === "complete") {
          ...
          console.log(
            chalk.cyan(">  Chain ID:"),
            statusResponse.result.chainId
          );
          console.log(
            chalk.cyan(">  Receipt Index:"),
            statusResponse.result.receiptIndex
          );
          console.log(
            chalk.cyan(">  Log Index:"),
            statusResponse.result.logIndex
          );
          const duration = (Date.now() - startTime) / 1000;
          console.log(
            chalk.cyan(">  Time to Complete:"),
            `${duration.toFixed(2)} seconds`
          );
          break;
        }
